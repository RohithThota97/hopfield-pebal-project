...
def random_mirror(img, gt=None):
    if random.random() >= 0.2:
        ...
        if gt is not None:
            # Ensure gt is numpy array and has correct dtype
            if isinstance(gt, Image.Image):
                gt = np.array(gt)
            # Ensure gt is proper numpy array with correct dtype
...
def SemanticEdgeDetector(gt):
    # Ensure gt is proper numpy array
    if isinstance(gt, Image.Image):
        gt = np.array(gt)
    if not isinstance(gt, np.ndarray):
        gt = np.array(gt)
    ...
    if gt.ndim > 2:
        gt = gt.squeeze()
    assert gt.ndim == 2, f"gt must be 2D array, got {gt.ndim}D with shape {gt.shape}"
    
    # Create copy and mask 255 values
    no255_gt = gt.copy().astype(np.uint8)
    no255_gt[gt == 255] = 0
...
class TrainPre(object):
    ...
    def __call__(self, img, gt=None):
        ...
        if isinstance(img, Image.Image):
            img = np.array(img)
        if gt is not None and isinstance(gt, Image.Image):
            gt = np.array(gt)
...
def get_mix_loader(engine, collate_fn=None, augment=True, cs_root=None,
    ...
    train_dataset = CityscapesCocoMix(split='train', preprocess=train_preprocess, cs_root=cs_root,
                                      coco_root=coco_root, subsampling_factor=0.5)
...
class LostAndFound(Dataset):
    ...
    def __init__(self, split='test', root="", transform=None):
        assert os.path.exists(root), "lost&found dataset not found"
        """Load all filenames."""
        ...
        # Modified to load from 'original' instead of 'leftImg8bit/self.split'
        images_root = os.path.join(root, 'original')
        labels_root = os.path.join(root, 'labels')  # Assume labels in 'labels' dir with same name

        if not os.path.exists(images_root):
            raise RuntimeError(f"LostAndFound images directory not found: {images_root}")
        
        for filename in os.listdir(images_root):
            if os.path.splitext(filename)[1] == '.png':
                filename_base = os.path.splitext(filename)[0]
                img_path = os.path.join(images_root, filename)
                label_path = os.path.join(labels_root, filename_base + '.png')  # Assume same name for labels
                
                if os.path.exists(label_path):
                    self.images.append(img_path)
                    self.targets.append(label_path)
                    # Optional annotations if needed
                    self.annotations.append(label_path)  # Or adjust if different
                else:
                    print(f"Warning: Missing label for image {filename}")
...
class Fishyscapes(Dataset):
    ...
    def __init__(self, split='Static', root="", transform=None):
        ...
        if split in ['Static', 'LostAndFound']:
        # Use separate original and labels dirs with flexible pairing
            images_root = os.path.join(root, 'original')
            labels_root = os.path.join(root, 'labels')
            if not os.path.exists(images_root) or not os.path.exists(labels_root):
                raise RuntimeError(f"Fishyscapes {split} directories not found in {root}")
            ...
            for filename in sorted(os.listdir(images_root)):
                ...
                # Try multiple label naming patterns
                    label_path = None
                    base_name = filename.replace('_leftImg8bit.png', '').replace('_unknown_unknown_leftImg8bit.png', '')
                
                # Pattern 1: exact filename match
                    candidate1 = os.path.join(labels_root, filename)
                # Pattern 2: *_unknown.png
                    candidate2 = os.path.join(labels_root, base_name + '_unknown.png')
                # Pattern 3: *_gtFine_labelIds.png  
                    candidate3 = os.path.join(labels_root, base_name + '_gtFine_labelIds.png')
        ...
        # Keep existing logic for other splits
            all_files = sorted([f for f in os.listdir(root) if f.endswith('.png')])
            for filename in all_files:
                if '_unknown_unknown_leftImg8bit.png' not in filename and filename.endswith('_leftImg8bit.png'):
                    f_name = filename
                    img_path = os.path.join(root, f_name)
                    label_f = f_name.replace('_leftImg8bit.png', '_unknown_unknown_leftImg8bit.png')
                    label_path = os.path.join(root, label_f)
                    if not os.path.exists(label_path):
                        label_f_alt = f_name.replace('_leftImg8bit.png', '_unknown.png')
                        label_path = os.path.join(root, label_f_alt)
                    if os.path.exists(label_path):
                        self.images.append(img_path)
                        self.targets.append(label_path)

        if len(self.images) == 0:
            print(f"Warning: No images loaded for Fishyscapes split '{split}' in root '{root}'")
    def __len__(self):
        """Return number of images in the dataset split."""
        return len(self.images)
...
class RoadAnomaly(Dataset):
    ...
    def __init__(self, root="", transform=None):
        ...
        self.images = []  # list of all raw input images
        self.targets = []  # list of all ground truth TrainIds images
        images_dir = os.path.join(root, 'original')
        labels_dir = os.path.join(root, 'labels')  # Assume labels in 'labels'
        ...
        for filename in filenames:
            if os.path.splitext(filename)[1] == '.jpg':
                ...
                label_path = os.path.join(labels_dir, f_name + '.png')  # Assume .png labels
                if os.path.exists(label_path):
                    self.images.append(img_path)
                    self.targets.append(label_path)
                else:
...
class Cityscapes(Dataset):
    ...
    def __init__(self, root: str = "", split: str = "val", mode: str = "gtFine",
        ...
        """
        Cityscapes dataset loader - Modified for custom directory structure
        """
        self.root = root
        self.split = split
        ...
        if not os.path.exists(target_dir):
            raise RuntimeError(f"Targets directory not found: {target_dir}")

        for file_name in os.listdir(img_dir):
            if file_name.endswith('_leftImg8bit.png'):
                # Based on your files, the target should be _gtFine.png (trainIds)
                target_name = file_name.replace('_leftImg8bit.png', '_gtFine.png')
                img_path = os.path.join(img_dir, file_name)
                target_path = os.path.join(target_dir, target_name)

                if not os.path.exists(target_path):
                    # Fallback to _gtFine_labelIds.png if exists
                    target_name_alt = file_name.replace('_leftImg8bit.png', '_gtFine_labelIds.png')
                    target_path = os.path.join(target_dir, target_name_alt)
                ...
                if os.path.exists(img_path) and os.path.exists(target_path):
                    ...
                    print(f"Warning: Missing files - img: {os.path.exists(img_path)}, target: {os.path.exists(target_path)}")
                    print(f"  Expected target: {target_path}")
...
class COCO(Dataset):
    ...
    def __init__(self, root: str, proxy_size: int, split: str = "train",
        ...
        annotations_path = os.path.join(self.root, "annotations", "ood_seg_" + self.split)
        images_path = os.path.join(self.root, self.split)
        
        if not os.path.exists(annotations_path):
            raise RuntimeError(f"COCO annotations directory not found: {annotations_path}")
        if not os.path.exists(images_path):
            raise RuntimeError(f"COCO images directory not found: {images_path}")

        for root, _, filenames in os.walk(annotations_path):
            ...
            for filename in filenames:
                if os.path.splitext(filename)[-1] == '.png':
                    ...
                    # Only add if both files exist
                    if os.path.exists(target_path) and os.path.exists(image_path):
                        self.targets.append(target_path)
        ...
        if len(self.images) == 0:
            print(f"Warning: No images loaded for COCO split '{self.split}' in root '{self.root}'")
    ...
    def __getitem__(self, i):
        ...
        try:
            ...
            print(f"Error loading COCO data at index {i}: {e}")
            print(f"Image: {self.images[i]}")
...
class CityscapesCocoMix(BaseDataset):
    ...
    def __init__(self, split='train', preprocess=None,
                 cs_root='',
                 coco_root="",
                 subsampling_factor=0.5, cs_split=None, coco_split=None, ):
    ...
    def mix_object(self, current_labeled_image=None, current_labeled_mask=None,
        ...
        if current_labeled_image is None:
            city_scape_idx = random.randint(0, self.city_number - 1)  # Fixed off-by-one error
        ...
        if isinstance(cut_object_mask, Image.Image):
            cut_object_mask = np.array(cut_object_mask)
        if isinstance(cut_object_image, Image.Image):
        ...
        if ood_boxes.size == 0:  # No objects to mix
        ...
        if mask.shape[0] != 0 and mask.shape[1] != 0:
            if cut_object_mask.shape[0] < current_labeled_mask.shape[0] and cut_object_mask.shape[1] < current_labeled_mask.shape[1]:
        ...
        try:
            ...
            if gt_path is not None:
                if not os.path.exists(gt_path):
                    print(f"Warning: Ground truth file does not exist: {gt_path}")
                    # Create dummy gt with same spatial dimensions as image
            ...
            print(f"Error fetching data from {img_path}, {gt_path}: {e}")
            raise e
